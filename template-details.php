...
<div class="property_details">
    ...
    <?php if(count($listing) > 0) { // check listing exists/found ?>
        ...
        <?php if(count($photos) > 0 && $photos[0]["URL"]["value"] != $wp_plugin_aem_params['AEM_PLUGIN_URL']."/images/no_image_available.jpg") { ?>	
            ...
            <div id="images_full" align="center">
				...
				<?php foreach($photos as $photo){ ?>
                    <?php $nphoto++; ?>
					<?php if($nphoto <= 1) { ?>
                        <img src="<?php echo $photo["URL"]["value"]; ?>" title="" alt="" />
                    <?php } ?>
                <?php } ?>
            </div>
            ...
            <?php if(count($photos) > 1) { ?>	
                <div id="images_thumbnails">
                    <?php foreach($photos as $photo){ ?>
                        <a href="<?php echo $photo["URL"]["value"]; ?>" style="text-decoration:none;">
                            <img width="70" src="<?php echo $photo["ThumbnailURL"]["value"]; ?>" title="" alt="" />
                        </a>
                    <?php } ?>
                    <div class="clear">&nbsp;</div>
                </div>
       ...
       <div class="clear" style="height:30px;">&nbsp;</div>
        ...
        <!-- AddThis Button BEGIN (remove addthis_32x32_style to have small icons) -->
